...
joker_samples.write(f"{DATA_PATH}/rejection_samples_{mils}M_{id_num}_{num_chains}chains_MWE_2.hdf5", overwrite = True) #writing out posterior samples (not MCMC)
...
plt.savefig(f"{DATA_PATH}/RVvTime_{mils}M_{id_num}_{num_chains}chains_MWE_2.png")
...
fig1.savefig(f"{DATA_PATH}/RVCurves_{mils}M_{id_num}_{num_chains}chains_MWE_2.png") #saving figure to plots folder in script output folder
...
fig2.savefig(f"{DATA_PATH}/PeriodvsEccent_{mils}M_{id_num}_{num_chains}chains_MWE_2.png") #saving figure to plots folder in script output  folder 
...
if len(joker_samples) == 1: 
    ...
    plt.title(f"ID: {id_num}, K1%={K1st}")
    fig3.savefig(f"{DATA_PATH}/RVCurves_MCMC_{mils}M_{id_num}_{num_chains}chains_MWE_2.png") #saving figure to plots folder in script output  folder
    ...
    ax4.set_ylabel("$e$")
    plt.title(f"ID: {id_num}, K1%={K1st}")
    fig4.savefig(f"{DATA_PATH}/PeriodvsEccent_MCMC_{mils}M_{id_num}_{num_chains}chains_MWE_2.png") #saving figure to plots folder in script output  folder
