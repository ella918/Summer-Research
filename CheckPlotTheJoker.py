...
def PlotTheJoker(id_num):
    print(id_num)
    if os.path.exists(f"{workpath}/{id_num}/rejection_samples_50.0M_{id_num}.hdf5") == False: #checking if the joker has been run on this object already
    ...
    fig1.savefig(f"{workpath}/RVCurves_{id_num}_new") #saving figure to plots folder in script output folder
    ...
    fig2.savefig(f"{workpath}/PeriodvsEccent_{id_num}_new") #saving figure to plots folder in script output  folder 
    print("Period vs Eccentricity plotted")

    if len(joker_samples) == 1: 
    	mcmc_samples = tj.JokerSamples.read(f'{workpath}/{id_num}/rejection_samples_MCMC_50.0M_{id_num}.hdf5')

    	fig3, ax3 = plt.subplots()
    	_ = tj.plot_rv_curves(mcmc_samples, data=data) #plotting RV curves from MCMC rejection sampler
    	plt.title(f"ID: {id_num}, K1%={K1st}")
    	fig3.savefig(f"{workpath}/RVCurves_MCMC_{id_num}_new") #saving figure to plots folder in script output  folder
    	...
    	ax4.set_ylabel("$e$")
    	plt.title(f"ID: {id_num}, K1%={K1st}")
    	fig4.savefig(f"{workpath}/PeriodvsEccent_MCMC_{id_num}_new") #saving figure to plots folder in script output  folder
    	print("Period vs Eccentricity from MCMC plotted")
    plt.close('all')
    return

PlotTheJoker(2128124963389008384)
