...
def IDtoRVWithOld(id_num):
    ...
    if len(rvdatamatched6811) == 0: #if there is nothing from 6811 then use the data from 6866 to create RVData obj
        matched1 = rvdatamatched6866
    if len(rvdatamatched6866) == 0: #if there is nothing from 6866 then use the data from 6811 to create RVData obj
        matched1 = rvdatamatched6811
    t1 = Time(matched1["hjd"], format = "jd", scale = "tcb") #create time object

    gname = matched1['DR3Name'][0] #getting name of object
    ...
    t_all1 = Time(np.append(t1, t2)) #merge time objects
    all_data= vstack([matched1, new]) #merge tables so all rv data is together 
    ...
    t3 = Time(matched["DATE-OBS"], format = "fits", scale = "tcb")

    for rvs in matched['vrad']:
        all_data['rv'].append(rvs)

    for rverrs in matched['vrad_err']:
        all_data['rve'].append(rverrs)

    t_all = Time(np.append(t_all2, t3))

    data = tj.RVData(t = t_all, rv = all_data["rv"]*(u.kilometer/u.second), rv_err = all_data["rve"]*(u.kilometer/u.second)) #create RVData obj
    return data #return the RVData obj and how many samples for the star


def RunTheJokerWithOld(id_num, num_priors):
    data = IDtoRVWithOld(id_num)
...
if __name__ == "__main__":
    ...
    RunTheJokerWithOld(args.id, args.prior)
