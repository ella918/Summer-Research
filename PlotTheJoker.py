...
unimodal_table = QTable.read('/data2/labs/douglste-laf-lab/mathewea/Summer-Research/unimodalcheck_200M_new.csv')
#id_list = Table.read('/data2/labs/douglste-laf-lab/mathewea/Summer-Research/GAIADR3_IDs.csv')
id_list = [2081880397683498368, 2128067445187794816, 2128099983858866816, 2128110257413561984, 2128127506009807232, 2128142830453283200, 2128145544872361984]
def PlotTheJoker(id_num):
    print(id_num)
    if os.path.exists(f"{workpath}/{id_num}/rejection_samples_200.0M_{id_num}_new.hdf5") == False: #checking if the joker has been run on this object already
    ...
    joker_samples = tj.JokerSamples.read(f"{workpath}/{id_num}/rejection_samples_200.0M_{id_num}_new.hdf5")
    ...
    fig1.savefig(f"{workpath}/RVCurves_{id_num}_200M_new") #saving figure to plots folder in script output folder
    ...
    fig2.savefig(f"{workpath}/PeriodvsEccent_{id_num}_200M_new") #saving figure to plots folder in script output  folder 
    print("Period vs Eccentricity plotted")

    if len(joker_samples) == 1: 
    	mcmc_samples = tj.JokerSamples.read(f'{workpath}/{id_num}/rejection_samples_MCMC_200.0M_{id_num}_new.hdf5')
    	#df = mcmc_samples.tbl.to_pandas()
    	#fig6, ax6 = plt.subplots()
    	#_= corner.corner(df)
    	#fig6.savefig(f"{workpath}/Corner_MCMC_{id_num}_200M_old")
    	fig3, ax3 = plt.subplots()
    	_ = tj.plot_rv_curves(mcmc_samples, data=data) #plotting RV curves from MCMC rejection sampler
    	plt.title(f"ID: {id_num}, K1%={K1st}")
    	fig3.savefig(f"{workpath}/RVCurves_MCMC_{id_num}_200M_new") #saving figure to plots folder in script output  folder
    	...
    	ax4.set_ylabel("$e$")
    	plt.title(f"ID: {id_num}, K1%={K1st}")
    	fig4.savefig(f"{workpath}/PeriodvsEccent_MCMC_{id_num}_200M_new") #saving figure to plots folder in script output  folder
    	print("Period vs Eccentricity from MCMC plotted")

    	fig5, ax5 = plt.subplots()
    	_=tj.plot_phase_fold(joker_samples, data)
    	fig5.savefig(f"{workpath}/Phase_Fold_MCMC_{id_num}_200M_new")
